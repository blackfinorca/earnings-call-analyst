...
ANTHROPIC_MODEL_LABEL = "Claude Sonnet 4.5"
...
def build_user_message(
    ...
    return (
        ...
        "```\n\n"
        "Return only the final markdown report for "
        "`M2 Sector ranking/sector-ranking-report.md`. "
        "Do not wrap the answer in Markdown code fences and do not add commentary outside the report. "
        "Keep the full response concise enough to fit roughly 2-3 pages of text."
    )
...
def main() -> None:
    ...
    output_path.write_text(response_text.strip() + "\n", encoding="utf-8")
